...
public class SubmitForApproval {
    ...
    public void getAndPushOrder(String payloadXML,String url, String type) throws Exception {
        pushOrder(payloadXML,url,type);
    }

    protected void pushOrder(String xmlData,String url, String type) throws MalformedURLException,
                                                    IOException {
        String xmlInput = "";
//        String url =
//            "http://" + pod + "/soa-infra/services/default/SellPaymentProcess/sellpaymentprocess_client_ep?WSDL";
        ...
        OkHttpClient client;
        client = new OkHttpClient.Builder().connectTimeout(600, TimeUnit.SECONDS).writeTimeout(600, TimeUnit.SECONDS).readTimeout(600, TimeUnit.SECONDS).build();

        MediaType mediaType = MediaType.parse("text/xml");
        RequestBody body = RequestBody.create(mediaType, xmlInput);
        Request request =
            new Request.Builder().url(url).post(body).addHeader("content-type", "text/xml").addHeader("cache-control", "no-cache").build();
