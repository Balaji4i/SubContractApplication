...
public class applicationAMImpl extends ApplicationModuleImpl implements applicationAM {
    ...
    public void createNewApplication(){
        ...
        //System.err.println("Total applicationLine Count--"+getXxscPayApplLinesVO1().getEstimatedRowCount());
        double TotalRowAppCount=getXxscPayApplLinesVO1().getEstimatedRowCount();
        //Equal to 0
            if(getXxscPayApplLinesVO1().getEstimatedRowCount()==0){
                ...
                getAppSearchPreviousROVO1().setNamedWhereClauseParam("BV_CONT_HRDID", hid);
                getAppSearchPreviousROVO1().setNamedWhereClauseParam("BV_VER", version);
                getAppSearchPreviousROVO1().setNamedWhereClauseParam("BV_CONT_LNEID", lid);
                getAppSearchPreviousROVO1().setNamedWhereClauseParam("APP_HRDID", appHrdRow.getAttribute("ApplHeaderId"));
                getAppSearchPreviousROVO1().executeQuery();
                //                        previousCalculationVO.getEstimatedRowCount();
                //System.err.println("Pre Qty, % Amt===COUNT=="+getAppSearchPreviousROVO1().getEstimatedRowCount());                        
                if(getAppSearchPreviousROVO1().getEstimatedRowCount()==0){
                    totalPer=0;
                    totalQty=0;
                    totalAmt=0;
                }else{
                    totalQty=getAppSearchPreviousROVO1().first().getAttribute("ApplPrevQty")==null?0:Double.parseDouble(getAppSearchPreviousROVO1().first().getAttribute("ApplPrevQty").toString());
                    totalPer=getAppSearchPreviousROVO1().first().getAttribute("ApplPrevPerc")==null?0:Double.parseDouble(getAppSearchPreviousROVO1().first().getAttribute("ApplPrevPerc").toString());
                    totalAmt=getAppSearchPreviousROVO1().first().getAttribute("ApplPrevAmount")==null?0:Double.parseDouble(getAppSearchPreviousROVO1().first().getAttribute("ApplPrevAmount").toString());
                    //System.out.println("totalQty---"+totalQty+"totalPer-----"+totalPer+"totalAmt---"+totalAmt);
                }
...
//                    if(TotalRowAppCount==count){
                        ...
                        appLineRow.setAttribute("PrevTotQty", new BigDecimal(totalQty));
                        appLineRow.setAttribute("PrevAmount", new BigDecimal(totalAmt));
                        appLineRow.setAttribute("OrgId", ADFContext.getCurrent().getSessionScope().get("ordID"));
...
//                }
    ...
    //System.err.println("==APP==PAGE 0=="+ADFContext.getCurrent().getSessionScope().get("page"));
        ...
        /*
            ...
            */
        //System.err.println("==Apps==PAGE END=="+ADFContext.getCurrent().getSessionScope().get("page"));
    ...
    /**
     * Container's getter for AdvanceValidations_ROVO1.
     * @return AdvanceValidations_ROVO1
     */
    public ViewObjectImpl getAdvanceValidations_ROVO1() {
        return (ViewObjectImpl)findViewObject("AdvanceValidations_ROVO1");
    }

    /**
     * Container's getter for applicationHrdROVO1.
     * @return applicationHrdROVO1
     */
    public ViewObjectImpl getapplicationHrdROVO1() {
        return (ViewObjectImpl)findViewObject("applicationHrdROVO1");
    }
    ...
    /**
     * Container's getter for getContMaxVersn_RoVo1.
     * @return getContMaxVersn_RoVo1
     */
    public ViewObjectImpl getgetContMaxVersn_RoVo1() {
        return (ViewObjectImpl)findViewObject("getContMaxVersn_RoVo1");
    }
}
